...
INITIAL_ARTICLES = [
    ...
    ('1664 BLANCHE', 45),
    ('MAGNERS CIDRE', 7),
    ('CHAMPAGNE MOET & MUMM', 5),
    ('RUINART BRUT', 7),
    ('RUINART BLANC', 1),
    ('B & B BLANC DE BLANC', 4),
    ('DON PERIGNON', 1),
    ('PROSECCO', 46),
    ('DEUTZ 1,5L', 2),
    ('ROSE STUDIO', 18),
    ('BLANC HIRONDELLE', 11),
    ('BLANC CHARDONNAY', 10),
    ('ROUGE PIC SAINT LOUP', 13),
    ('ROUGE COTE DU RHONE', 10),
    ('PEPSI', 16),
    ('TONIC', 22),
    ('LIMONADE', 39),
    ('MEGA FORCE', 46),
...
def build_excel_report(personne, mouvements_valides):
    ...
    filename = datetime.now().strftime("Jukeboxx_Stock_%Y-%m-%d_%H-%M-%S.xlsx")
    output = BytesIO()
...
st.markdown('<div class="app-title">🍾 Jukeboxx</div>', unsafe_allow_html=True)
st.markdown(
